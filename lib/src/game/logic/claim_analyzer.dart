...
class ClaimAnalyzer {
  ...
  bool canClaimRemainingTricks() {
    ...
    final totalOtherCards =
        otherHands.values.fold(0, (sum, hand) => sum + hand.length);
  ...
  bool _simulateAllTricks(_GameSituation situation) {
    ...
    Position leader;
    if (situation.currentTrick != null &&
        situation.currentTrick!.plays.isNotEmpty) {
  ...
  _TrickSimulationResult _simulateCurrentTrick(
    _GameSituation situation,
  ...
  _TrickSimulationResult _simulateTrick(
    _GameSituation situation,
    Position leader,
    TrickEngine trickEngine,
  ) {
    var trick =
        Trick(plays: [], leader: leader, trumpSuit: trumpRules.trumpSuit);
  ...
  PlayingCard _chooseBestCard(
    ...
    if (player == Position.south) {
      ...
      return _chooseWeakestWinningCard(legalCards, trick, trickEngine) ??
          _chooseWeakestCard(legalCards);
    } else {
  ...
  PlayingCard? _chooseWeakestWinningCard(
    List<PlayingCard> cards,
    Trick trick,
    TrickEngine trickEngine,
  ) {
    final currentWinner = trickEngine.getCurrentWinner(trick);
    final currentWinningCard = currentWinner != null
        ? trick.plays.firstWhere((p) => p.player == currentWinner).card
        : null;
  ...
  PlayingCard _chooseStrongestCard(
    List<PlayingCard> cards,
    Trick trick,
    TrickEngine trickEngine,
  ) {
